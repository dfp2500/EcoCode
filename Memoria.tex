...
Para intentar estandarizar estas mediciones, la Green Software Foundation ha desarrollado la especificación Software Carbon Intensity (SCI), que nos permite calcular cuánto CO$_2$ genera un sistema de software \cite{ref5}. La fórmula es bastante directa:
...
\textbf{Las entradas que necesitas:}

\begin{itemize}
\item \textbf{E (Energy consumption – Consumo de energía)}: El consumo energético del hardware donde se ejecuta tu software, medido en kilovatios-hora (kWh)
...
\item \textbf{I (Carbon Intensity – Intensidad de carbono)}: Cuánto CO$_2$ genera la red eléctrica de tu región por cada kWh consumido (se mide en gramos de CO$_2$ equivalente por kilovatio-hora (gCO$_2$e/kWh))
...
\item \textbf{M (Carbono incorporado)}: Las emisiones asociadas a la fabricación y eliminación del hardware donde se ejecuta el software (servidores, ordenadores, etc.) Se expresa generalmente en kgCO$_2$e (kilogramos de CO$_2$ equivalente)
...
\textbf{Lo que obtienes:}
El resultado es una estimación que te dice cuánto CO$_2$eq genera tu software por cada unidad de trabajo (por ejemplo, gramos de CO$_2$eq por usuario o por petición). Cuanto más detallados, específicos y fiables sean los datos de entrada, mas se parecerá esta estimación a la realidad. Cuanto más bajo sea el número, mejor para el medio ambiente. Aunque es imposible llegar a 0 (siempre vas a consumir algo de energía), te ayuda a comparar diferentes soluciones y ver si vas mejorando \cite{ref6}.
...
En esta sección se definen los criterios fundamentales para la evaluación de herramientas destinadas a la medición de la huella de carbono del software. Estos criterios están orientados a garantizar que la herramienta sea precisa, de fácil uso, accesible para distintos perfiles de usuario y técnicamente robusta.

\subsection*{1. Precisión y Fiabilidad del Cálculo}
\begin{itemize}
    \item Utilizar estándares reconocidos y aceptados internacionalmente, como \textbf{GHG Protocol}, \textbf{ISO 14064} u otros modelos científicamente validados.
    \item Basarse en modelos de estimación documentados y contrastados (por ejemplo, \textit{OneByte} o \textit{Sustainable Web Design}).
    \item Posibilidad de contextualizar los cálculos en función de la \textbf{intensidad de carbono regional}.
    \item Transparencia metodológica, detallando claramente los pasos de conversión de datos a emisiones y sus limitaciones.
\end{itemize}

\subsection*{2. Cobertura y Alcance}
\begin{itemize}
    \item Calcular las emisiones generadas por la ejecución local del software y, de forma opcional, permitir estimar las emisiones en entornos cloud.
    \item Capacidad de incluir los tres alcances definidos por GHG Protocol: \textbf{Scope 1}, \textbf{Scope 2} y \textbf{Scope 3}.
    \item Desglose granular de las emisiones por región, proceso o recurso utilizado.
    \item Opción de proyectar emisiones futuras o simular escenarios hipotéticos de reducción.
\end{itemize}

\subsection*{3. Facilidad de Integración y Uso}
\begin{itemize}
    \item Disponibilidad de diferentes interfaces: interfaz web intuitiva, línea de comandos (CLI), API REST, etc.
    \item Flexibilidad en la instalación, pudiendo ejecutarse de forma local o en contenedores.
    \item Integración con procesos de desarrollo, de forma que se pueda automatizar su ejecucion cuando se produzcan cambios en el software.
    \item Visualización interactiva de los resultados, con paneles comprensibles, tablas y gráficos.
    \item Interfaz de usuario intuitiva que permita a los usuarios introducir los datos de forma sencilla y clara.
    \item Minimizar dependencias de terceros proveedores sin alternativas de respaldo para asegurar la continuidad y estabilidad del sistema.
\end{itemize}

\subsection*{4. Accesibilidad y Usabilidad}
\begin{itemize}
    \item Baja curva de aprendizaje, de forma que pueda ser utilizada por usuarios no técnicos a un nivel simple.
    \item Configuración simplificada mediante asistentes o procedimientos guiados.
    \item Resultados comprensibles y adaptados a distintos perfiles de usuario (usuarios no técnicos, responsables de sostenibilidad, perfiles técnicos).
    \item Inclusión de comparativas y equivalencias contextualizadas (por ejemplo, emisiones expresadas en kilómetros recorridos en coche o consumo energético doméstico).
    \item Sistema de puntuación y ranking de los resultados, de forma que se pueda saber claramente si el resultado obtenido es bueno o malo.
    \item Consejos prácticos y personalizados para la mejora de la huella de carbono del software analizado, orientados a distintos perfiles de usuario.
\end{itemize}

\subsection*{5. Capacidad de Exportación y Reporte}
\begin{itemize}
  \item Permite guardar los datos en formatos sencillos y compatibles, como hojas de cálculo (CSV) o archivos de texto estructurados (JSON).
  \item Genera informes que se pueden personalizar según las necesidades, por ejemplo, para auditorías o presentaciones dentro de la empresa.
  \item Mantiene un historial de los datos, facilitando ver cómo cambian las emisiones a lo largo del tiempo.
\end{itemize}
